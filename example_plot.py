...
def env_barplot(env_title, plot_title):
    ...
    # Draw bar plot
    labels = [entry['algo-nickname'] for entry in deduped_entries]
...
def remove_duplicates(entries):
    ...
    for i, entry in enumerate(entries):
        ...
        if entry['algo-nickname'] == next_entry['algo-nickname']:
            if entry['score'] == next_entry['score']:
                duplicate_indices.append(i + 1)
            else:
                print('[WARN] Algorithm ({}) with same nickname have different scores ({}, {}) from source ({}, {}). Consider renaming them.'.format(entry['algo-nickname'], entry['score'], next_entry['score'], entry['source-nickname'], next_entry['source-nickname']))
